...
class GCN(Model):
    ...
    def _loss(self):
        ...
        # d = get_normalized_vector(d, True, x.indices, x.dense_shape) * 1e-6
        # logit_p = logit
        # logit_m = self.get_output(tf.sparse_add(x, d))
        # dist = kl_divergence_with_logit(logit_p, logit_m)
        # grad = tf.gradients(dist, [d.values], aggregation_method=2)[0]
        # d = tf.stop_gradient(grad)
        #
        # r_vadv = get_normalized_vector(d, True, x.indices, x.dense_shape) * 0.03
        # #self.r_vadv = r_vadv
        #
        # logit_p = tf.stop_gradient(logit)
        # logit_m = self.get_output(tf.sparse_add(x, r_vadv))
        # ll = kl_divergence_with_logit(logit_p, logit_m)
